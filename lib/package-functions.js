...
const isFileOrDirectory = (pathFile) => {
  try {
...
const responseHttpRequestAsync = (httpx, link) => {
  ...
  return new Promise((resolve, reject) => {
    const httpModuleOriginal = protocols[httpx];
    try {
      const req = httpModuleOriginal.get(link, (resp) => {
        let httpModule;
        let url;
...
const validateMdLinkAsync = (mdLinks) => new Promise((resolve, reject) => {
...
const statsMdLinks = (mdLinksArray) => {
  ...
  if (totalMdLinks >= 1) {
    ...
    const hrefDuplicates = hrefLinks.filter((href, index) => hrefLinks.indexOf(href) !== index);
...
const validateMdLinkArray = (mdLinksArray) => new Promise((resolve, reject) => {
  ...
  Promise.all(validateLinks)
    .then((res) => {
      ...
      const linksValid = res.filter((link) => link.status === 200);
      resolve([
        linksBroken.length,
        linksRedirect.length,
        linksBrokenRedirect.length,
        linksValid.length]);
    })
    .catch((error) => reject(error));
});
...
const statsAndValidateLinks = (mdLinksArray, totalAndUniquesLinks) => {
  ...
  return new Promise((resolve, reject) => {
    validateMdLinkArray(mdLinksArray).then((res) => {
      ...
      validLinks.valid = res[3];
      resolve({
        ...totalAndUniquesLinks,
        ...brokenLinks,
        ...redirectLinks,
        ...brokenRedirectLinks,
        ...validLinks,
...
const validateMdLinksArray = (mdLinksArray) => new Promise((resolve, reject) => {
  const validateLinks = mdLinksArray.map(validateMdLinkAsync);
  Promise.all(validateLinks)
